  ...
  const handleLike = async () => {
    ...
    await runTransaction(firestore, async (transaction) => {
      ...
      if (userDoc.exists() && problemDoc.exists()) {
        if (liked) {
          // remove problem id from likedproblems on user document, document likes on problem document
          ...
          <div className="w-full">
            ...
            {!loading && currentProblem && (
              <div className="flex items-center mt-3">
                <div
                  className={`${problemDifficultyClass} bg-olive inline-block rounded-[21px] bg-opacity-[.15] px-2.5 py-1 text-xs font-medium capitalize `}
                >
